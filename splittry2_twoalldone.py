...
while True:
    ...
    if event.type == MOUSEBUTTONDOWN:
        ...
        if x[index] > 368  and 432 > x[index] and 102 > y[index] and 38 < y[index]:
            x[index]=  400
            y[index] = 70
        elif x[index] > 368  and 432 > x[index] and 202 > y[index] and 138 < y[index]:
            x[index]=  400
            y[index] = 140
    ...
    if check[0] == 1:
            ...
            x[0] -= loaded[0].get_width()/2
            y[0] -= loaded[0].get_height()/2
    if check[1] == 1:
            x[1],y[1] = pygame.mouse.get_pos()
            x[1] -= loaded[1].get_width()/2
            y[1] -= loaded[1].get_height()/2
    ...
    for k in range(2):
        for f in range(2):
             if x[k] == 400 and y[k] == 70 + 70*f:
                 stored[f]=k

    tex = 0

    path = []
    for mcount in range(2):
        mcounter = mcount+1
